...
const getFormattedAst = (nodesThree, prefix = '') => {
  const outputVariants = {
    ...
    changed: (node, prefixInside) => {
      const dataType = `${_.isObject(node.oldValue)}${_.isObject(node.newValue)}`;
      const outputs = {
        truefalse: `Property '${prefixInside}${node.name}' was updated. From '[complex value]' to ${addQuotesToStringValue(node.newValue)}\n`,
        falsetrue: `Property '${prefixInside}${node.name}' was updated. From ${addQuotesToStringValue(node.oldValue)} to '[complex value]'\n`,
        falsefalse: `Property '${prefixInside}${node.name}' was updated. From ${addQuotesToStringValue(node.oldValue)} to ${addQuotesToStringValue(node.newValue)}\n`,
      };
      return outputs[dataType];
    },
    nested: (node, prefixInside) => `${getFormattedAst(node.children, `${prefixInside}${node.name}.`)}`,
  };
